// 共有パスワード方式のアクセス制限（order-intake-ai-demo と同方式）。
...
export async function passwordOk(submitted: string): Promise<boolean> {
  ...
  const a = await hmacHex("rrd-pw-compare", submitted);
  const b = await hmacHex("rrd-pw-compare", pw);
